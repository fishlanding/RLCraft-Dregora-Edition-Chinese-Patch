            ...
            if (!isNull(event.item.tag.Potion)) {
                if ((event.item.tag.Potion == "potioncore:flight") || (event.item.tag.Potion == "potioncore:long_flight")) {
                    if (event.player.y <= 70) {
                        ...
                        event.player.sendStatusMessage("你感觉到一股神秘的力量从下方涌来，猛地将药剂从你手中夺走", true);
                        event.cancel();
                    }
                }
            }
    ...
    if (!isNull(HeldArmor[0])) {
        ...
        if (HeldArmor[0].name == "item.srparasites.armor_boots") {
            ...
            if (isNull(HeldArmor[0].tag.srpkills)) {
                ...
                var SRPKills = ("§r§9---> " + totalHealth + "§r§5§o 你的武器已尝过鲜血，如今渴望着寄生虫……") as string;
                ...
                if TotalNewKills >= 50000 {
                    ...
                    var SRPKills = ("§r§9---> " + TotalNewKills + "§r§5§o 你的武器已尝过鲜血，如今渴望着寄生虫……") as string;
                    HeldArmor[0].mutable().updateTag({display:{Lore:[SRPKills]}});
...
events.onEntityLivingDeath(function(event as EntityLivingDeathEvent){
    ...
    if ((event.entity.definition.id) has "srparasites") {
        ...
        if (!isNull(HeldEquipment[0].tag.srpkills)) {
            ...
            HeldEquipment[0].mutable().updateTag({display:{Lore:["","你的武器已尝过鲜血，如今渴望着寄生虫……"]}});
...
for item in conductivity_1 {item.addTooltip("§e§o导电性 +1");}
for item in conductivity_2 {item.addTooltip("§e§o导电性 +2");}
for item in conductivity_3 {item.addTooltip("§e§o导电性 +3");}
    ...
    if ((entityBiome == "Abyssal Rift") || (entityBiome == "Parasite Biome")) {
        ...
        if (!isNull(event.player.getRidingEntity())) {
            ...
            if (isNull(event.player.nbt.ForgeData.lightning_cooldown_abyssal)) {
                ...
                event.player.sendStatusMessage("一种不祥的感觉笼罩着你，仿佛有人在暗中窥视……或许最好步行前进。", true);
                event.player.sendStatusMessage("一种不祥的感觉笼罩着你，仿佛有人在暗中窥视……或许最好步行前进。", false);
            }
